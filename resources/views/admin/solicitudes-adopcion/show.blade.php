<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Detalle de solicitud de adopción</h2></x-slot>
    <div class="py-12"><div class="max-w-3xl mx-auto sm:px-6 lg:px-8"><div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <dl class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
            <div><dt class="font-medium text-gray-500">Ciudadano</dt><dd class="mt-1 text-gray-900">{{ $solicitud->solicitante->nombres }} {{ $solicitud->solicitante->apellidos }}</dd></div>
            <div><dt class="font-medium text-gray-500">Correo electrónico</dt><dd class="mt-1 text-gray-900">{{ $solicitud->solicitante->email }}</dd></div>
            ...
            <div class="sm:col-span-2"><dt class="font-medium text-gray-500">Comentarios del ciudadano</dt><dd class="mt-1 whitespace-pre-line text-gray-900">{{ $solicitud->comentarios_solicitante ?? '—' }}</dd></div>
            @if ($solicitud->comentarios_admin)<div class="sm:col-span-2"><dt class="font-medium text-gray-500">Comentarios administrativos</dt><dd class="mt-1 whitespace-pre-line text-gray-900">{{ $solicitud->comentarios_admin }}</dd></div>@endif
        </dl>
        <a href="{{ route('admin.solicitudes-adopcion.index') }}" class="mt-6 inline-block text-sm text-gray-600 hover:text-gray-900">Volver al listado</a>
    </div></div></div>
</x-app-layout>
